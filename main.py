...
client = MongoClient('mongodb+srv://wavehq:wavehq@cluster0.l0fcj.mongodb.net/hqtrivia?retryWrites=true&w=majority')
db = client.get_database("hqlives_db")
database = db.user
lifebase = db.life
number_base = db.number
pending_base = db.pending
numbers_base = db.numbers
white = [489724998831964170,475658166852648970]#Discord ID For Owner
black = [489724998831964170,475658166852648970]#Discord ID2 For Owner
red = [489724998831964170,475658166852648970] #Stock Adder ID
def rand():
...
async def add_point(ctx, amount:int):
    addchannel = bot.get_channel(741366190462599268)
    commander_id = ctx.message.author.id
...
async def give_point(ctx, amount:int , user: discord.Member):
    givelog = bot.get_channel(741366190462599268)
    user_in_list = True
    commander_id = ctx.message.author.id
    user_id = user.id
    id_list = []
...
async def on_member_join(member):
    ...
    ch = bot.get_channel(741366190462599268)
...
bot.run('NzgyNjUwMDI0ODIyOTY0Mjg1.X8PRog.0mjoAn8ybV0Zoktxz7GFhGD_v8g')
